 ...
 *
 * **9였을 때 이 게임에는 전투가 없었다.** 양쪽 AI로 20분씩 다섯 판을 돌리면
 * 전사자가 다섯 판 합쳐 여덟 명이고, 본진은 900:900으로 한 대도 안 맞은 채
 * 끝났다. 두 부대를 10만 떨어뜨려 세우면 60초 동안 서로를 못 본 척한다.
 *
 * 값이 근접 사거리(방패병끼리 3.2 + 2.3×1.75 ≈ 7.2)보다 겨우 컸던 것이
 * 원인이다. 몸이 닿기 직전까지 와야 표적으로 잡히는데, 지역 한 변이 60이라
 * 양쪽이 같은 지역에 서 있어도 20~30씩 떨어져 있는 것이 보통이다. 방어하러
 * 온 부대가 침입자와 같은 땅에 서서 서로를 못 보고 판이 끝났다.
 *
 * 22는 세 가지를 동시에 만족한다 — 근접 사거리의 약 3배, 우클릭으로 흩어
 * 세운 부대 한 무리의 너비(고리 반지름 3.2·6.4·9.6이라 열 명이면 13쯤)보다
 * 넓고, 지휘 반경(30)보다는 좁다. 마지막 조건이 중요하다: 지휘 반경이 여전히
 * 더 큰 원이어야 "반경 안에서 세진다"가 전투보다 큰 개념으로 남는다.
 *
 * 다섯 시드로 재보면 전사자가 8명 → 235명, 승부가 난 판이 0/5 → 2/5가 된다.
 * 아직 3/5가 시간 초과인 것은 **따로 남은 문제다** — 부대가 싸우기는 하는데
 * 본진까지 뚫지 못한다. 이 상수로 고칠 수 있는 것이 아니다.
 */
const AGGRO = 22
