...
export class Cameras {
   ...
   *
   * 유닛이 점이 되지 않는 선에서 잡는다 — 발밑 링이 보여야 지휘 반경 규칙이
   * 전달된다(GDD 6.2). 아래위로 열어 두되, 멀어지는 쪽은 맵이 딱 들어오는
   * 데까지만 간다(`capFor`).
   *
   * **168에서 96으로 당겼다.** 168은 화면 짧은 쪽에 맵 세로(180)를 거의 그대로
   * 담는 값이라, 기본 배율이 곧 "맵 전체"였다. 그 하나에서 2회차의 지적 셋이
   * 한꺼번에 나온다:
   *
   * - **밀 곳이 없다.** 초점이 갈 수 있는 범위가 1280×720에서 ±31·±23까지
   *   줄어, 150/초로 미는 손이 0.2초 만에 벽에 닿았다. 키보드도 화면 가장자리도
   *   미니맵도 같은 상자에 갇히므로 셋 다 "눌러도 안 움직인다"가 된다.
   * - **물러설 곳이 없다.** 상한까지 휠 한 칸 반이라 굴려도 화면이 안 변한다.
   * - **유닛이 먼지다.** 720픽셀에 168을 담으면 1월드가 4.3픽셀이라 병사 몸통이
   *   열 픽셀이고 창이 지르는 궤적선은 한 픽셀 아래로 내려간다. 전투 표현이
   *   있는데 화면에 닿지를 못한다.
   *
   * 96이면 1월드가 7.5픽셀이 되어 몸통이 두 배가 되고, 초점 범위가 ±82·±52로
   * 열리고, 상한까지 아홉 칸이 남는다. 화면에 담기는 것은 지역 두어 개다 —
   * 판 전체를 늘 보여 주는 일은 미니맵이 맡는다.
   */
  private static readonly DEFAULT_SPAN = 96
  /**
   * 가장 가까이 볼 수 있는 배율.
   *
   * 45면 1월드가 16픽셀이라 창 궤적과 막아 낸 금색 불꽃이 그대로 읽힌다.
   * 2회차 플레이어가 "화면을 확대해 잘라 보니 있더라"고 적은 그 그림을
   * 게임 안에서 만들 수 있어야 한다.
   */
  private static readonly MIN_SPAN = 45
   ...
   * 1.08이면 하한에서 상한까지 열여덟 칸쯤 되어 손으로 고를 수 있다.
   */
