 ...
 * 정확히 그것이다.
 *
 * 설계 의도 — 여덟 기물은 각자 명확한 "못 하는 것"을 갖는다.
 ...
 *   거마작   딜은 거의 없고 한 번에 하나만 붙잡지만, 기마는 여기서 반쯤 멈춘다
 *   별파진   순수 지속 피해라 무엇에게든 통하지만 중첩되지 않는다
 *   기고     딜이 0이고, 주변에 기물이 없으면 아예 아무것도 하지 않는다
 * 어느 하나만 도배해서는 마지막 스테이지를 넘길 수 없도록 수치를 잡았다.
 *
 * **광역 둘(살수·화차)과 단일 둘(사수·포수)이 서로를 대체하지 않도록** 축을
 * 하나씩 어긋나게 뒀다 — 살수는 가장 싸고 가장 짧고 방어에 가장 약하고,
 * 화차는 넓지만 보병 전용. 사수는 싸고 짧고 저뎀, 포수는 비싸고 길고 고뎀.
 *
 * **곱셈 축은 두 방향이다** — 거마작은 적을 느리게, 기고는 아군을 빠르게.
 * 배치 판단이 정반대라 서로를 대체하지 않는다: 거마작은 **적이 가장 오래
 * 머무는 곳**에, 기고는 **아군이 모인 곳**에 놓아야 값을 한다. (거마작을
 * 경로 앞쪽에 두라는 예전 규칙은 틀린 것으로 판명됐다 — docs/BALANCE.md 8장.)
 */
...
export const TOWER_DEFS: Record<string, TowerDef> = {
  ...
  frost: {
    ...
    desc: '통나무에 창을 꽂은 방책. 한 번에 하나씩만 붙잡지만 쉬지 않고 걸고, 죽이지는 못해도 그 몇 초가 뒤쪽 기물의 사격 기회를 늘린다. 이미 묶인 것을 다시 붙잡느라 낭비하지 않고 감속이 풀릴 때가 된 쪽으로 옮겨 간다. 기마에게는 훨씬 깊게 걸린다 — 거마작은 애초에 말을 막으라고 만든 물건이다.',
    levels: [
      ...
      // 판단이 선명해진다.
      //
      // **광역을 떼고 단일 대상으로 바꿨다.** 그냥 splashRadius만 0으로 두면
      // 곧바로 망가진다 — S5가 100% → 8%로, 거마작을 아예 안 넣은 62%보다도
      // 나빠졌다. 한 발에 하나만 묶이니 기병 물량을 못 잡는 것인데, 진짜
      // 낭비는 타겟팅에 있었다: 「선두」 기준이라 이미 묶인 맨 앞 하나를
      // 계속 다시 쐈다. Tower.selectTarget이 "감속이 곧 풀릴 적"을 고르게
      // 하고(자세한 이유는 그쪽 주석), 광역을 잃은 만큼만 연사로 보상했다
