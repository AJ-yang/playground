 ...
 *   armor 높음        → 관통(사수·포수)이 깎임 → 화약(총통)이 필요
 *   magicResist 높음  → 화약(총통)이 깎임 → 관통 기물(사수·포수)이 필요
 *   flying(기마)      → 화차가 아예 조준하지 못함
 *   speed 높음        → 거마작 감속 없이는 사거리 안에 머무는 시간이 부족
 * 한 종류만 도배하면 반드시 막히도록 웨이브를 섞는 것이 밸런스의 축이다.
 *
 * **필드 이름은 옛 것이고 화면에 뜨는 말은 새 것이다.** `armor`/`magicResist`/
 * `flying`은 중세 판타지 시절의 식별자가 그대로 남은 것이고, 플레이어가 보는
 * 이름은 갑주 / 산개 / 기마다. 사람이 읽는 문자열을 새로 적을 때는 반드시
 * 뒤쪽을 쓸 것 — 식별자를 그대로 화면에 옮겼다가 웨이브 태그가 말 탄 병사를
 * 「공중」이라고 부르고 있었다.
 */
