...
const RULES: readonly Rule[] = [
  ...
  /**
   * 위의 두 `notWorse`는 **차이**만 본다 — 셋 다 0%로 같이 무너지면 초록인 채로
   * 지나간다. 대조군 규칙에는 언제나 이 구멍이 있어서 바닥을 하나 깔아 둔다.
   *
   * 바닥값의 근거는 GDD 9장 첫 줄이다 — *"각 스테이지가 그때 가진 기물로
   * 풀리는가? **풀린다.** 적응형 빌드가 S1~S6 전부 95~100%"*. 스테이지가 자기
   * 해금만으로 풀리지 않으면 4.5의 해금 설계가 통째로 무너지므로, 이건 지표가
   * 아니라 전제다.
   *
   * **조기 소환을 쓰지 않는 두 빌드로만 검사한다.** GDD 9장은 *"조기 소환
   * 2G/초가 과한가? 아니다. 클리어 여부를 뒤집지 않고 남은 생명만 올린다"* 라고
   * 답해 뒀다 — 조기 소환이 후반의 **입장료**가 되면 그 답이 뒤집힌다.
   * `adaptive-early`를 여기 넣지 않는 이유가 그것이다.
   *
   * **3기는 여기서도 뺀다.** 위와 같은 이유로 과투자는 의도된 손해다
   * (GDD 4.2). 바닥을 까는 대상은 최적 구간 1~2기뿐이다.
   */
  {
    kind: 'floor',
    what: '적응형 빌드는 그때 가진 기물만으로 후반을 넘는다 (조기 소환 없이)',
    stages: LATE,
    strategies: ['adaptive-frost1', 'adaptive-frost2'],
    min: 0.9,
  },
  {
    kind: 'notWorse',
